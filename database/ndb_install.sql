...
rem
rem NAME
rem   ndb_install.sql - Main installation script for NDB schema creation
rem
rem DESCRIPTION
rem   NDB (Nutrition Database) is a comprehensive schema 
rem   designed to function as a structured repository for nutritional facts of foods, 
rem   taking into account for alterations resulting from different cooking methods. 
rem
rem SCHEMA VERSION
rem   1
rem
...
rem   Oracle 21c and higher
rem
rem MAJOR CHANGES IN THIS RELEASE
rem   new script for NDB installation
rem
rem SCHEMA DEPENDENCIES AND REQUIREMENTS
rem  This script calls ndb_create.sql, ndb_populate.sql, ndb_code.sql
rem 
rem INSTALL INSTRUCTIONS
rem   1. Run as privileged user with rights to create another user
rem      (SYSTEM, ADMIN, etc.)
rem   2. Run this script to create the NDB (Nutrition Database) schema
...
rem   If you have installed the NDB sample schema, you can remove it by running
rem   the ndb_uninstall.sql script
rem
rem NOTES
...
PROMPT
PROMPT Thank you for installing the Oracle Nutrition Database Schema.
...
PROMPT The entire installation will be logged into the 'ndb_install.log' log file.
PROMPT
...
SPOOL ndb_install.log

rem =======================================================
rem Accept and verify schema password
rem =======================================================

ACCEPT pass PROMPT 'Enter a password for the user NDB: ' HIDE

BEGIN
   IF '&pass' IS NULL THEN
      RAISE_APPLICATION_ERROR(-20999, 'Error: the NDB password is mandatory! Please specify a password!');
   END IF;
END;
/
...
ACCEPT tbs PROMPT 'Enter a tablespace for NDB [&var_default_tablespace]: ' DEFAULT '&var_default_tablespace'
...
rem =======================================================
rem cleanup old NDB schema, if found and requested
rem =======================================================
...
BEGIN
   SELECT MAX(username) INTO v_user_exists
      FROM all_users WHERE username = 'NDB';
   ...
   IF v_user_exists IS NOT NULL THEN
      ...
      IF UPPER('&overwrite_schema') = 'YES' THEN
         ...
         DBMS_OUTPUT.PUT_LINE('Old NDB schema has been dropped.');
...
rem =======================================================
rem create the NDB schema user
rem =======================================================

CREATE USER ndb IDENTIFIED BY &pass
...
ALTER SESSION SET CURRENT_SCHEMA=NDB;
ALTER SESSION SET NLS_LANGUAGE=American;
ALTER SESSION SET NLS_TERRITORY=America;

rem =======================================================
rem create NDB schema objects
rem =======================================================
...
/*
SELECT 'nutrients' AS "Table", 392 AS "provided", count(1) AS "actual" FROM ndb.customers
UNION ALL
SELECT 'stores' AS "Table", 23 AS "provided", count(1) AS "actual" FROM ndb.stores
UNION ALL
...
SELECT 'The installation of the schema is now finished.'  AS "Thank you!"
   FROM dual
UNION ALL
...
SELECT '' AS "Thank you!"
   FROM dual
UNION ALL
SELECT 'Thank you for using Nutrition Database!' AS "Thank you!"
   FROM dual
UNION ALL
SELECT '' AS "Thank you!"
   FROM dual;
